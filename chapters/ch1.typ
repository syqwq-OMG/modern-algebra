...
#definition([线性空间])[
  ...
  + $forall alpha in V, exists ! beta in V$ s.t. $alpha+beta=beta+alpha=vv$，且 $-alpha eq^(triangle.stroked.small) beta$
  + $(x y)alpha = x(y alpha)$
...
#proof()[
  ...
  $
    cases(
      n+0 eq^(triangle.stroked.small) n,
...
#lemma([代数元的和与积])[
  若 $alpha, beta$ 是 $F$ 上的代数元，则 $alpha+beta, alpha beta$ 也是 $F$ 上的代数元。
]
#proof[
...
#proof[
  设 $[E:K]=m <oo, [K:F]=n<oo$，则由 @field-dim-formular
  $
    [E:F]=[E:K] dot [K:F]=m dot n <oo
  $
]
...
== 代数闭包
#definition([（相对）代数闭包])[
  设 $E\/F$，则 $K=lr({ alpha in E mid(|) alpha "在" F "上代数"})$ 是 $E\/F$ 的中间域，称 $K$ 为 $F$ 在 $E$ 中的 *（相对的）代数闭包*。
]
#remark()[
  若 $K$ 是 $F$ 在 $E$ 上的代数闭包， $K'$ 是 $E\/K$ 的中间域，且 $K'\/K$ 是代数的，则 $K'=K$.
]
#proof[
  $K'\/K, K\/F$ 都是代数的，则 $K'\/F$ 也是代数的，从而 $K'=K$.
]
#example()[
  + $E_1=RR, F_1=QQ, K_1=lr({ "实代数数" })$

  + $E_2=CC, F_2=QQ, K_2=lr({ "复代数数" })$
]
#definition([代数闭域])[
  若 $K$ 没有真代数扩张，则称 $K$ 是*代数闭域*。
]
也就是说任何以 $K$ 为系数的多项式都在 $K$ 上有根。
#example()[
  + $CC$ 是代数闭域。

  + $RR$ 不是代数闭域，因为 $x^(2)+1$ 在 $RR$ 上没有根。

  + $QQ$ 不是代数闭域，因为 $x^(2)-2$ 在 $QQ$ 上没有根。
]
...
#theorem()[
  $E$ 是一个代数闭域， $F subset.eq E$，且 $K$ 是 $F$ 在 $E$ 中的相对代数闭包，则 $K$ 是 $F$ 的一个绝对代数闭包。
]
#proof[
  由相对代数闭包的定义， $K\/F$ 是代数扩张，只需证明 $K$ 是代数闭域即可。

  设 $K'\/K$ 是一个代数扩张，则有域塔 $F subset.eq K subset.eq K' => K' \/ F "也是代数扩张。"$ $forall alpha in K'$，因为 $alpha$ 也是 $F$ 上的代数元，因此 $exists F "上的极小多项式" m(x) in F[x]$ s.t. $m(alpha)=0$，又 $F subset.eq E => m(x) in E[x]$. 接下来，因为 $E$ 是代数闭域，考虑 $E(alpha)$，则 $E(alpha) \/ E "是代数的" => E(alpha) = E => alpha in E$. 由 $alpha$ 是 $F$ 上的代数元，且 $alpha in E$，可知 $alpha in K$。也就是 $forall alpha in K', alpha in K$，得到 $K'=K$.
]
...
== Galois 群
这边他讲的顺序好迷啊。。。怎么这么难啊 #qwq

#definition([对称群])[
  设 $X$ 是一个集合，定义 $S(X)=lr({ sigma:X -> X mid(|) sigma "是双射" })$，运算复合 $compose :S times S -> S, (sigma, tau) mapsto sigma compose tau$，运算逆 $(space)^(-1):S -> S, sigma mapsto sigma^(-1)$，单位元 $e:X -> X, x mapsto x$.
  若满足：
  + $(sigma compose tau) compose mu = sigma compose (tau compose mu)$

  + $sigma compose e=e compose sigma = sigma$

  + $sigma compose sigma^(-1) = sigma^(-1) compose sigma = e$
  则称 $(S,compose )$ 为 $X$ 上的*对称群*.
]

#definition([置换群])[
  若 $S$ 是一个对称群，$G subset.eq S$ 满足 $forall sigma, tau in G$, $sigma compose tau in G$ , $sigma^(-1) in G$
  则称 $(G, compose )$ 为 $X$ 上的*置换群*。
]

#definition([自同构群])[
  $E$ 是一个域，则
  $
    Aut(E) eq^(triangle.stroked.small)
    lr(
      { sigma in S(E) mid(|) &sigma(0)=0, sigma(1)=1, \ &sigma(x+y)=sigma(x)+sigma(y), \
...
#definition([（抽象）群])[
  设 $G$ 是一个集合，定义 $compose :G times G -> G$,  $(space)^(-1):G -> G$, $e in G$.
  若满足：
  + $(sigma compose tau) compose mu = sigma compose (tau compose mu)$
...
#definition([子群])[
  $G$ 是一个群， $H subset.eq G$， 若 $forall sigma, tau in H$，满足：
  + $sigma compose tau in H$

  + $sigma^(-1) in H$
  则称 $H$ 是 $G$ 的一个*子群*，记作 $H <= G$。
]
...
#definition([Galois 群])[
  若 $E\/F$，则
  $
    Gal(E\/F)eq^(triangle.stroked.small)
    lr({ sigma in Aut(E) mid(|) sigma|_(F) = id|_(F) })
  $
  称为 $E\/F$ 的*Galois 群*。
]
...
#theorem()[
  $E\/F$ 是有限扩张，则 $Gal(E\/F)$ 是有限群。
]
#gemini()[
  *证明思路*
  + 一个 $F$ -自同构 $sigma in Gal(E\/F)$  是一个 $E -> E$  的映射，它由其如何作用于 $E$  的一组基来完全确定。

  + 因为 $E\/F$  是有限扩张，所以我们可以找到一组有限的基。

  + 我们将证明，对于基中的任何一个元素， $sigma$  只能将其映射到有限个可能的目标上。

  + 既然基是有限的，且每个基元素的可选目标也是有限的，那么总共可能的 $F$ -自同构的数量也必然是有限的。
]

#proof[
  #let gg = [$Gal(E\/F)$]
  设 $[E:F]=n$，且 $E$ 在 $F$ 上的一组基为 $u_1, u_2, dots.h.c , u_n$，也就是 $E=F(u_1, u_2, dots.h.c , u_n )$.

  那么，$sigma in #gg$，若 $forall u in E,
  u=c_1 u_1+c_2 u_2+ dots.h.c +c_n u_n$
  考虑他在 $sigma$ 的作用下的像：
  $
    sigma(u)=
    c_1 sigma(u_1)+c_2 sigma(u_2)+ dots.h.c +c_n sigma(u_n)
  $
  由于 $sigma$ 保持 $F$ 中的每个元素，所以 $E$ 中元素在 $sigma$ 下的像完全由基的像决定。

  接下来，考察 $sigma$ 对基作用的像的范围。因为 $[E:F]<oo$，因此也是代数扩张，对于 $u_i$ 他在 $F$ 中的极小多项式 $m(x)=b_k x^(k) +dots.h.c + b_1 x + b_0 in F[x]$，于是
  $
    sigma(m(u_i)) & =sigma(0)=0 \
                  & =sigma(b_k u_i^(k) +dots.h.c + b_1 u_i + b_0) \
                  & =b_k sigma(u_i)^(k)+dots.h.c + b_1 sigma(u_i)+b_0=m(sigma(u_i))
  $
  这说明： $u_i$ 经过 $sigma$ 作用之后一定还是 $m(x)$ 的根，又 $deg m<=[E:F]=n$，所以基中的一个元素最多有 $n$ 种可能，从而一个上界是
  $
    lr(| #gg |)<=n^(n)
  $
]
...
#definition([群的不动域])[
  设 $E$ 是一个域， $G <= Aut(E)$，定义
  $
    Inv(G) eq^(triangle.stroked.small)
    lr({ alpha in E mid(|) forall sigma in G, sigma(alpha)=alpha })
  $
  则 $Inv(G)$ 是 $E$ 的子域，称为 $G$ 的*不动域*。
]

#theorem([Artin 引理])[
  设 $E$ 是一个域， $G$ 是 $Aut(E)$ 的有限子群，记 $F=Inv(G)$，则
  $
  [E:F]<=lr(| G |)
  $  
  从而， $E\/F$ 是有限扩张。
...
#proof[
  设 $lr(| G |)=n, G=lr({ eta_1, eta_2, dots.h.c , eta_n })$，下证 $forall m > n$， $E$ 中的 $m$ 个元素都 $F$-线性相关。

  考虑 $E$ 中的 $m$ 个元素 $u_1, u_2, dots.h.c , u_m$，目标是寻找一组 $x_1, x_2, dots.h.c x_m$ s.t. 
  $
  x_1 u_1+x_2 u_2+ dots.h.c +x_m u_m= 0
  $    
  由于 $G$ 中的元素是 $E$ 的自同构且保持 $F$，考虑 $G$ 中的元素 $eta_j$ 的作用有：
  $
    eta_j (x_1 u_1+x_2 u_2+ dots.h.c +x_m u_m) = x_1 eta_j (u_1)+x_2 eta_j (u_2)+ dots.h.c +x_m eta_j (u_m)=0
  $   
  因为 $m>n$，该方程组一定在 $E$ 上有非零解。取所有解中非零元最少的解（零元最多的解） $X=(x_1, x_2,dots.h.c, x_m) in E^(m)$，不妨设 $x_1!=0$，进而可以假设 $x_1=1$，于是解选定为 $X=(1, x_2,dots.h.c, x_m)$，下面证明 $X in F$（此后定理证毕）。

  考虑反证法。假设 $exists x_i in.not F$，不妨设 $x_2 in.not F$，也就是说 $exists eta in G$ s.t. $eta(x_2)!=x_2$. 接下来，考察 $eta(X)=(1,eta(x_2),dots.h.c,eta(x_m))$，有如下事实：
  + $eta(X)$ 也是该方程组的解。考虑将 $eta$ 作用在方程组上：
  $
    eta(sum_(i=1)^(m) x_i eta_j (u_i)) = eta(0)=0 =>
    sum_(i=1)^(m) eta(eta_j (u_i)) eta(x_i) =0
  $ 
    因为 $eta compose eta_j in G$，所以当 $eta_j$ 遍历整个 $G$ 的时候， $eta compose eta_j$ 也会遍历整个 $G$，所以新方程组等价于原方程组，只是方程的顺序变了。
  
  2. $eta(X)$ 中零元的个数和 $X$ 中零元的个数相等。因为 $eta$ 是双射，所以 $eta(x_i)=0 <=> x_i=0$.
  3. $eta(X)-X$ 仍是方程组的非零解，但是零元更多。因为 $eta(x_2)!=x_2$，所以 $eta(X)-X$ 中第二个元素不为0，而第一个元素为0。这与 $X$ 的取法矛盾。证毕。
]

#problem([域-群-域])[
  设 $E\/F$ 是一个有限扩张， $G=Gal(E\/F)$ 是一个有限群， 且 $G <= Aut(E)$，令 $F'=Inv(G)$，则 $F'=F$ 吗？   
]
#solution()[
  不一定。首先有 $F subset.eq F'$，我们可以找出反例其实。 
]

#problem([群-域-群])[
  设 $E$ 是一个域， $G <= Aut(E)$ 是一个有限群， $F=Inv(G)$，则 $E\/F$ 是有限扩张，于是 $G'=Gal(E\/F)$ 是有限群， $G'=G$ 吗？   
]
#solution()[
  是。称为 Artin 定理。 
]

Galois 理论就是研究什么样的有限扩张 $E\/F$ 可以使得 $Inv(Gal(E\/F))=F$.  
